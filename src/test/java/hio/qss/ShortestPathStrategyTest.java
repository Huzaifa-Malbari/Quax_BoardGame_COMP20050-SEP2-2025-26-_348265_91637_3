...
class ShortestPathStrategyTest {
  @Test
  void testGetShortestPathBetween() throws ClassNotFoundException, NoSuchMethodException,
    ...
    path = AStar.getShortestPathBetween(
        state, new SearchNode(ocells[0][0]), new SearchNode(ocells[0][10]));
    assertEquals(11, path.size());

    path = AStar.getShortestPathBetween(
        state, new SearchNode(ocells[0][0]), new SearchNode(ocells[10][10]));
    assertEquals(21, path.size());

    path = AStar.getShortestPathBetween(
        state, new SearchNode(ocells[0][0]), new SearchNode(rcells[0][0]));
    assertEquals(2, path.size());

    path = AStar.getShortestPathBetween(
        state, new SearchNode(rcells[0][0]), new SearchNode(rcells[0][9]));
    assertEquals(11, path.size());

    path = AStar.getShortestPathBetween(
        state, new SearchNode(rcells[0][0]), new SearchNode(rcells[9][9]));
    assertEquals(19, path.size());
  }

  @Test
  void testAddNeighbourPaths()
    ...
    ArrayList<ArrayList<SearchNode>> paths =
        (ArrayList<ArrayList<SearchNode>>) pathsField.get(strategy);
  ...
  @Test
  void testAddPath() throws ClassNotFoundException, NoSuchMethodException, NoSuchFieldException,
